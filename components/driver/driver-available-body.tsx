...
export async function DriverAvailableBody() {
  ...
  if (!driverInfo?.is_available) redirect("/driver")
  if (!driverInfo?.guide_completed_at) redirect("/driver/guide")
    ...
    <DriverDeliveryRequestProvider>
      ...
      <Card className="border-0 shadow-sm w-full min-w-0">
        <CardHeader className="py-2 px-3 space-y-0">
          ...
          <CardDescription className="text-xs">내 위치 기준 가까운 순 최대 20건</CardDescription>
        </CardHeader>
