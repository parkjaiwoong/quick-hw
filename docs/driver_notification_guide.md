...
## 알림 대상 선정 (서버 쿼리) — "비활성"은 사용하지 않음

배차 신호를 보낼 때 서버는 **`is_available`(배송 가능)** 를 사용합니다. **`is_active` 등 "비활성" 필터는 없습니다.**

| 단계 | 조건 | 비고 |
|------|------|------|
| 1차 | RPC `find_nearby_drivers` | `is_available = true` + `current_location IS NOT NULL` + 출발지 **10km 이내**, 거리순 **최대 5명** |
| 2차 | **근처 5명 이하일 때** | RPC 결과(근처 1~5명) + **배송 가능 전원** 을 합쳐서 알림 (위치 없음 기사도 포함) |
| 3차 | 근처 6명 이상일 때 (RPC limit 상향 시) | 근처 기사만 알림 (현재 RPC limit=5 라서 항상 5명 이하 → 실질적으로 항상 전원 포함) |

- **근처 기사가 5명 이하**이면 **배송 가능(is_available=true) 전원**에게 FCM을 보냅니다. 위치가 없어도 포함됩니다.
- 서버 로그에서 `[기사알림-2] 근처 5명 이하 → 배송가능 전원에게 알림 (위치 없음 포함)` 과 `merged`, `available` 수를 확인하면 됩니다.
- **알림 제외**: `is_available = false` 인 기사만 제외됩니다. `notification_audit_log` 의 `merge_nearby_and_available` 이벤트에서 `driver_ids` 로 대상 목록을 확인할 수 있습니다.
...
- **기사 Flutter(9)**: `flutter run` 으로 앱을 실행한 **터미널** 또는 **Android Studio Logcat**(필터 `FCM`)에서 확인합니다.  
  **"Lost connection to device"** 가 나와 디버그 콘솔에 로그가 안 보일 때: **터미널 → 작업 실행(Tasks: Run Task)** 에서 **`adb-logcat-driver`** 를 실행하면, 기기 로그(Flutter `print` 포함)가 전용 터미널에 계속 출력됩니다. 디버그 실행 전에 이 작업을 켜 두면 연결이 끊겨도 로그를 볼 수 있습니다.
...
3. **「실시간 알림 구독 성공」만 보이고 UI/진동/소리 안 나올 때**  
   로그에 `[기사-Realtime] 실시간 알림 구독 성공` 만 있고, 고객 결제 후 `[기사-Realtime] INSERT 콜백 호출` 이 안 찍히면 **INSERT 이벤트가 기사 앱까지 오지 않은** 상태입니다. 순서대로 확인하세요.
   - **서버에서 알림 INSERT 했는지**: Vercel Dashboard → Logs에서 고객 결제 직후 `[기사알림-2] 근처 기사 수: N` 또는 `배송가능 기사로 대체: N` 이 **1 이상**인지, `[기사알림-3] notifications INSERT 성공` 이 나오는지 확인. 0명이면 서버가 해당 기사를 찾지 못한 것(배포 미반영, 배송 가능 꺼짐, 위치 없음 등).
   - **Realtime publication**: Supabase → Database → Replication 에서 `public.notifications` 가 포함돼 있는지 확인. 없으면 **`scripts/055_verify_realtime_notifications.sql`** 실행.
   - **기사 앱 콘솔**: WebView 로그에서 결제 후 `[기사-Realtime] INSERT 콜백 호출` 이 나오면 이벤트는 도착한 것이고, 그 다음 `[기사-Realtime] UI/진동/소리 실행` 또는 `INSERT 스킵(타입/ delivery_id 아님)` 으로 원인 구분.
...
4. **모바일에서 확인 (F12 없음)**  
   ...
   - **「실시간 알림 연결 실패」** (노란 배너) → Realtime 설정 문제 또는 **CHANNEL_ERROR**(WebView/네트워크). PC에서 Supabase SQL 실행 또는 관리자 문의. **CHANNEL_ERROR가 나와도 10초 폴링이 돌아가므로 알림은 최대 10초 안에 도착합니다.**
   - **PC에서만** F12 → Console로 상세 로그 확인 가능.

5. **DB에는 알림 row가 있는데 띵동/진동/모달이 안 뜰 때 (이벤트는 오는데 UI·소리만 막힌 경우)**  
...
6. **개발 환경에서 UI/진동/소리 반복 테스트 (10~20회)**  
